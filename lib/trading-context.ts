...
interface TradingEvent {
  ...
  kind: string;
  atMs: number;
  watchBeforeMs: number;
  freezeBeforeMs: number;
  freezeAfterMs: number;
  cooldownAfterMs: number;
}

export const DEFAULT_TRADING_EVENTS: TradingEvent[] = [
  {
...
function getKstParts(nowMs: number = Date.now()): KstParts {
...
export function getKoreaSessionState(nowMs: number = Date.now()): KoreaSessionState {
...
export function inferNextFundingTimeMs(nowMs: number = Date.now(), intervalHours: number = 4): number {
...
interface EventWindowResult {
  status: string;
  label: string;
  event: { key: string; label: string; atMs: number } | null;
  hoursToEvent: number | null;
  nextEvent: { key: string; label: string; atMs: number; hoursToEvent: number } | null;
  untilMs?: number;
...
export function getEventWindow(nowMs: number = Date.now(), events: TradingEvent[] = DEFAULT_TRADING_EVENTS): EventWindowResult {
  ...
  let nextEvent: { key: string; label: string; atMs: number; hoursToEvent: number } | null = null;
...
function bucketLast(rows: Record<string, unknown>[], bucketSec: number, priceSelector: (row: Record<string, unknown>) => number): Map<number, BucketRow> {
...
export function alignBasisSeries({
  spotTicks = [],
  binanceTicks = [],
  fxRate,
  bucketSec = DEFAULT_BASIS_BUCKET_SEC,
}: {
  spotTicks?: Record<string, unknown>[];
  binanceTicks?: Record<string, unknown>[];
  fxRate?: number;
  bucketSec?: number;
} = {}): BasisPoint[] {
  if (!Number.isFinite(fxRate) || fxRate <= 0) return [];
  const spotMap = bucketLast(spotTicks, bucketSec, row => (row.after_hours_price as number) || (row.price as number));
  const binanceMap = bucketLast(binanceTicks, bucketSec, row => row.price as number);
...
export function computeBasisSnapshot(series: BasisPoint[] = [], options: { lookback?: number; bandWidth?: number } = {}): BasisSnapshot {
