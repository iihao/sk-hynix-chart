...
function parseGoogleNewsRss(xml, positiveKeywords, negativeKeywords, options = {}) {
  ...
  const latestPublishedAt = displayItems.reduce((latest, item) => {
...
function mapByTimestamp(rows, mapper) {
  const map = new Map();
...
function unionTimestampSets(maps) {
  const set = new Set();
...
function mergeSentimentHistory({ lsData = [], takerData = [], oiData = [], topData = [] }) {
  const lsMap = mapByTimestamp(lsData, row => ({
    ls_ratio: parseFloat(row.longShortRatio) || 1,
    ls_long_pct: parseFloat(row.longAccount) || 0.5,
    ls_short_pct: parseFloat(row.shortAccount) || 0.5,
  }));
  const takerMap = mapByTimestamp(takerData, row => ({
    taker_ratio: parseFloat(row.buySellRatio) || 1,
    taker_buy_vol: parseFloat(row.buyVol) || 0,
    taker_sell_vol: parseFloat(row.sellVol) || 0,
  }));
  const oiMap = mapByTimestamp(oiData, row => ({
    open_interest: parseFloat(row.sumOpenInterest) || 0,
    oi_value: parseFloat(row.sumOpenInterestValue) || 0,
  }));
  const topMap = mapByTimestamp(topData, row => ({
    top_ls_ratio: parseFloat(row.longShortRatio) || 1,
    top_long_pct: parseFloat(row.longAccount) || 0.5,
    top_short_pct: parseFloat(row.shortAccount) || 0.5,
  }));

  const timestamps = unionTimestampSets([lsMap, takerMap, oiMap, topMap]);
  const state = { ls: null, taker: null, oi: null, top: null };
  const rows = [];
...
function buildFactorCoverage({ expectedFactors = [], factors = [], sourceHealth = [] }) {
...
function emptyOpenInterestSignal(reason, extras = {}) {
  return {
    score: 0,
    weight: 0,
